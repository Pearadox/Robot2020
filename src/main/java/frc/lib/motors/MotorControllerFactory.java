...
public class MotorControllerFactory {
    public static CANSparkMax createSparkMax(int canID, MotorConfiguration config) {
        ...
        if (config.feedbackDevice.device == FeedbackDevice.IntegratedSensor) {
            if (config.brushed) { throw new IllegalArgumentException("Only Neos can have integrated sensors"); }
            return motor;
        } else if (config.feedbackDevice.device == FeedbackDevice.QuadEncoder) { return motor; }
